/*************************************************************************
  ...
  > Created Time: 二 10/23 10:42:43 2018
 ************************************************************************/
...
#include <string.h>
#include <time.h>
#define DICT_HT_INIT_SIZE (16)
typedef enum dict_item_value_type
{
    ptr_type = 0,
    uint64_type,
    int64_type,
    double_type
} dict_item_value_type;
...
static uint64_t crc64_hash_func(const void *key, size_t len)
{
    const char *s = (char *)key;
    uint64_t crc;
...
static uint64_t jump_hash_func(uint64_t key, size_t num_buckets)
{
    int64_t b = -1, j = 0;
    while (j < num_buckets)
    {
...
inline static dict_item *dict_item_create(void *key, void *val, int type)
{
    if (type < ptr_type || type > double_type)
    {
        return NULL;
    }
    dict_item *di = (dict_item *)calloc(1, sizeof(*di));
    assert(di != NULL);
    di->key = key;
    uint64_t *uint_ptr;
    int64_t *int_ptr;
...
dict *dict_create(void *context, uint64_t size, int level_limit_size)
{
    ...
    assert(dt != NULL);
    size_t rsize = (size < DICT_HT_INIT_SIZE) ? DICT_HT_INIT_SIZE : size;
    dt->table = (dict_item **)calloc(rsize, sizeof(dict_item *));
    assert(dt->table != NULL);
    dt->table_size = rsize;
    dt->link_level_size = (level_limit_size <= 0) ? 0 : level_limit_size;
    dt->link_level = (uint8_t *)calloc(rsize, sizeof(uint8_t));
    dt->hash_func = NULL;
    return dt;
}
...
int dict_add(dict *dt, void *key, void *val, int item_type)
{
    ...
    uint64_t key_hash = (dt->hash_func == NULL) ? crc64_hash_func(key, dt->key_len(key)) : dt->hash_func(key, dt->key_len(key));
    uint64_t index = jump_hash_func(key_hash, dt->table_size);
    if (dt->link_level_size > 0 && (dt->link_level[index] == dt->link_level_size || dt->link_level[index] > (UINT8_MAX - 1)))
    {
        return -1;
    }
    dict_item *item = dict_item_create(key, val, item_type);
    if (item == NULL)
    {
        return -1;
    }
    item->key_hash = key_hash;
    if (dt->table[index] == NULL)
    {
        dt->table[index] = item;
    }
    else
    {
        item->next = dt->table[index];
        dt->table[index] = item;
        __sync_fetch_and_add(&dt->link_level[index], 1);
    }
    return 0;
}
void *dict_fetch(dict *dt, void *key)
{
    ...
    uint64_t key_hash = (dt->hash_func == NULL) ? crc64_hash_func(key, dt->key_len(key)) : dt->hash_func(key, dt->key_len(key));
    uint64_t index = jump_hash_func(key_hash, dt->table_size);
    dict_item *item = dt->table[index];

    while (item != NULL)
    {
        if (dt->key_cmp(item->key, key) == 0)
        {
            break;
        }
        item = item->next;
...
int dict_del(dict *dt, void *key)
{
    ...
    if (item == dt->table[index])
    {
        dt->table[index] = item->next;
    }
    ...
    if (dt->key_destroy != NULL)
    {
        dt->key_destroy(item->key);
    }
    if (dt->val_destroy != NULL && item->type == ptr_type)
    {
        dt->val_destroy(item->val.ptr);
    }
    free(item);
    __sync_fetch_and_sub(&dt->link_level[index], 1);
    return 0;
}
void dict_destroy(dict *d)
{
    for (uint64_t i = 0; i < d->table_size; i++)
    {
        dict_item *item = d->table[i];
        if (item == NULL)
        {
            continue;
        }
        while (item != NULL)
        {
            if (d->key_destroy)
            {
                d->key_destroy(item->key);
            }
            if (d->val_destroy && item->val.ptr != NULL)
            {
                d->val_destroy(item->val.ptr);
            }
            free(item);
            item = item->next;
        }
...
{
    uint64_t *ptr1 = (uint64_t *)key1;
    uint64_t *ptr2 = (uint64_t *)key2;
    if (*ptr1 == *ptr2)
    {
        return 0;
    }
...
{
    dict *d = dict_create(NULL, atoi(argv[1]), 0);
    d->key_len = &key_len;
    d->key_cmp =  &key_cmp;
    d->key_destroy = &free;
    ...
    for (uint64_t i = 1; i < times; i++)
    {
        ...
        *key = rand() * (rand() % i);
        int ret0 = dict_add(d, key, &i, 1);
        fprintf(stdout, ":::dict_add:  %d,", ret0);
        uint64_t *ret1 = dict_fetch(d, key);
        if (ret1 != NULL)
        {
            fprintf(stdout, "  dict_fetch:  %ld,", *ret1);
        }
        int ret2 = dict_del(d, key);
        fprintf(stdout, "  dict_del:%d:::\n", ret2);
    }
    dict_destroy(d);
}
#endif
