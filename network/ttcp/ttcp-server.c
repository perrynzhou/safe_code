...
void handle_connection(void *arg) {
  ...
  fprintf(stdout, "server got session_msg->{number=%d,length=%d}\n",
          req->sm.number, req->sm.length);
  payload_msg pm;
  pm.length = req->sm.length;
  for (int i = 0; i < req->sm.number; i++) {
    read_len = read_n(req->cfd, &pm, sizeof(pm));
    assert(read_len != sizeof(pm));
    // block function
    int32_t v = htonl(pm.length);

    write_len = write_n(req->cfd, &v, sizeof(int32_t));
    assert(write_len != sizeof(int32_t));
  }
  char sock_info[128] = {'\0'};
  get_sock_info(req->cfd, (char *)&sock_info);
  fprintf(stdout, "------ finish  thread %ld write to %s------\n",
